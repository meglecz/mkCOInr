...
# 	Keep genes if no motif similar to COI in gene or proteine name
...
(
	...
	'min_length' => 700, # minimum length of the cleaned sequence; 100 for COI, 1000 from COI from mitogenome
	'max_length' => 999999999999,  # maximum length of the cleaned sequence; 2000 for both COI and mitogenome
	'outdir' => ''
);
modify_params_from_tags(\%params, \@ARGV);

my @regexp_gene = ('cox*.*[1i]'),
my @regexp_protein = ('cytochrome', 'cox*.*[1i]'), # case insensitive
...
while(my $line = <IN>)
{
	...
	if($line =~ /^>/)# new sequence
	{
		...
		if($line =~ /^>([^\s]+).*\[gene\=([^\]]+)\].*\[protein\=([^\]]+).*\[location\=([^\]]+)/) 
		{
			...
			my $protein = $3;
			my $location = $4;
			### gene name similar to COI
			if(check_regexp($gene, \@regexp_gene, 0)) 
			{
#				print GENE $gene, "\n";
				next;
			}
			++$stat{'02. Total number of sequences without regular expression in gene name: '};
			### protein name similar to COI
			if(check_regexp($protein, \@regexp_protein, 0)) 
			{
#				print PROT $protein, "\n";
				next;
			}
			++$stat{'03. Total number of sequences without regular expression in protein name: '};
...
#		print OUT "$tax_seq{$taxid}{$seq}	$taxid	$seq_id{$tax_seq{$taxid}{$seq}}	$seq\n";
		###
